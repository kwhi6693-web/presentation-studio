...
1. Before any Python, Node, or Git probe or CLI call, use the host's documented dependency/runtime resolver when one is available. Do not assume a particular Agent API or loader name. A Codex App workspace-dependency loader is one optional host adapter; it is not part of the core Skill contract.
2. Resolve runtimes without PATH ambiguity. Retain the host resolver's absolute Python, Node, Git, and Node-package paths; if it is unavailable or fails, follow the exact copy-safe block in [dependencies.md](references/dependencies.md), which invokes `scripts/resolve-runtimes.ps1`. That block implements the remaining preflight and recommend handoff below. Never probe PATH first and never use bare `python`, `py`, `node`, or `git`; a WindowsApps Python alias is not runtime evidence. Mark runtime `PARTIAL / NOT EXECUTED` only when both the host resolver and checked-in resolver fail.
...
4. Run `scripts/preflight.py` with the resolved absolute Python and Node executables and pass the loader's Node-package directory through `--node-modules` when available. Use its module-level capability map instead of assuming that an executable alone provides every optional engine feature. Missing optional providers, packages, or desktop applications are recoverable capability limits, not an automatic task failure.
...
Use `PASS`, `PARTIAL`, or `FAIL` for production and quality results. A check is `PASS` only after its command ran successfully in the current environment. State every absent or unexecuted provider, browser, font, PowerPoint, LibreOffice, narration, animation, or visual-regression capability as `NOT AVAILABLE` or `PARTIAL / NOT EXECUTED`. These are capability-level results, not whole-Skill compatibility verdicts. Do not announce completion until required outputs exist, the quality loop closed, and all selected-engine validations were rerun after the final repair.
